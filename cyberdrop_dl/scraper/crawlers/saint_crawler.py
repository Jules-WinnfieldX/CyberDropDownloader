...
class SaintCrawler:
    ...
    async def startup(self) -> None:
        """Starts the crawler"""
        self.scraper_queue = await self.manager.queue_manager.get_scraper_queue("saint")
        self.download_queue = await self.manager.queue_manager.get_download_queue("saint")
    ...
    async def video(self, scrape_item: ScrapeItem) -> None:
        ...
        async with self.request_limiter:
            soup = await self.client.get_BS4("saint", scrape_item.url)
    ...
    async def handle_file(self, url: URL, scrape_item: ScrapeItem, filename: str, ext: str) -> None:
        ...
        check_complete = await self.manager.db_manager.history_table.check_complete("saint", url)
        if check_complete:
            await log(f"Skipping {url} as it has already been downloaded")
            await self.manager.progress_manager.download_progress.add_previously_completed()
            return

        download_folder = await get_download_path(self.manager, scrape_item, "Saint")
